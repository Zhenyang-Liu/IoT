...
static const sr_cmd_t *app_sr_cmd_from_id(int id)
{
    sr_cmd_t *it;
...
void sr_handler_task(void *pvParam)
{
        ...
        while (true) {
            ...
            if (ESP_MN_STATE_DETECTED & result.state) {
                        const sr_cmd_t *item = app_sr_cmd_from_id(result.command_id);
            
                        if (item) {
                                ESP_LOGI(TAG, "Recognised \"%s\" (id=%u cmd=%d)",
                                    item->str, (unsigned)item->id, item->cmd_id);
                        } else {
                                ESP_LOGW(TAG, "Unknown cmd id=%d", result.command_id);
                        }
                        bool handled = true;  
